...
public class KnightTourCritter extends KnightCritter
{
    ...
    /**
     * Determines, out of the potential moves, which one moves 
     * toward a location
     * with the lowest accessibility. It will return the location 
     * of first move
     * with the corresponding accessibility from the accessibility
     *  grid that is
     * the lowest. If there is a tie, it will return the location
     *  of smaller
     * move value in the list of locations. If all accessibility 
     * values are < 0,
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        for ( int i = 0; i < locs.size(); i++ )
        {
            ...
            if ( accessGrid[row][col] < lowestAccess )
            {
    ...
    /**
     * loc contains the coordinates of the latest square to be m
     * arked on the
     * access grid. The accessibility numbers of all valid moves
     *  reachable from
     * loc are decreased by 1. This algorithm will allow accessi
     * bility numbers
     * to go negative.
     * 
     * @param loc
     *            location of the latest square to be 
     *            marked on the access grid
     */
    public void updateAccess( Location loc )
    {
        ArrayList<Location> ar = getPossibleMoves(loc);
        for( int i = 0; i < ar.size(); i++ ) 
        {
            Location curr = ar.get( i );
            accessGrid[curr.getRow()][curr.getCol()]--;
        }
     ...
     *  to pick the possible
     * location with lowest accessibility, or to return the
     *  current location
     * (getLocation()) if <code>locs</code> has size 0. Aft
     * er the best move has
     * been chosen, the accessibility of the moves from the
     *  current location are
     * updated <br />
     * 
     * Postcondition: (1) The returned location is an e
     * lement of
     * <code>locs</code>, this critter's current locatio
     * n, or <code>null</code>.
